				...
				case 'login' :
					...
					if( !is_user_logged_in() ) {
						...
						if ( $errors === false ) {
							...
							switch ( $type ) {
								case 'registration' :
									$reg_string = esc_html__( 'Registrar', 'shopkit' );
								break;
								case 'login' :
									$reg_string = esc_html__( 'Login', 'shopkit' );
								break;
								default :
									$reg_string = esc_html__( 'Login / Registrar', 'shopkit' );
								break;
							}
						}
						else {
						...
						$string .= '<span>Saiba aqui como criar sua loja virtual grátis</span>';

						echo '<a href="/criar-loja-gratis" title="Criar sua loja virtual gratuitamente" class="shopkit-login-icon' . $class . ' ' . $icon_type . '">' . $string . '</a>';
...
<?php
		...
		public static function login_form_fields() {
			...
			ob_start(); ?>
				<h3><?php esc_html_e('Login', 'shopkit'); ?></h3>
